...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BIGKEYCOIN address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BIGKEYCOINAddressEntryValidator(parent));
    widget->setCheckValidator(new BIGKEYCOINAddressCheckValidator(parent));
}
...
bool parseBIGKEYCOINURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BIGKEYCOINUnits::parse(BIGKEYCOINUnits::BGKC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBIGKEYCOINURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBIGKEYCOINURI(uriInstance, out);
}

QString formatBIGKEYCOINURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("Bigkeycoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BIGKEYCOINUnits::format(BIGKEYCOINUnits::BGKC, info.amount, false, BIGKEYCOINUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BIGKEYCOIN.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BIGKEYCOIN (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BIGKEYCOIN (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BIGKEYCOIN*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BIGKEYCOIN\n";
        else
            optionFile << strprintf("Name=BIGKEYCOIN (%s)\n", chain);
