...
// Copyright (c) 2012-2021 The BIGKEYCOIN Core developers
...
void AddrManImpl::Serialize(Stream& s_) const
{
     ...
     *   * BIGKEYCOIN Core version N knows how to parse up to format=3. If a new format=4 is
     *     introduced in version N+1 that is compatible with format=3 and it is known that
     *     version N will be able to parse it, then version N+1 will write
     *     (format=4, lowest_compatible=3) in the first two bytes of the file, and so
     *     version N will still try to parse it.
     *   * BIGKEYCOIN Core version N+2 introduces a new incompatible format=5. It will write
