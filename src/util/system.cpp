...
// Copyright (c) 2009-2021 The BIGKEYCOIN Core developers
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BIGKEYCOIN
    // macOS: ~/Library/Application Support/BIGKEYCOIN
    // Unix-like: ~/.bitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BIGKEYCOIN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BIGKEYCOIN";
#else
    // Unix-like
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BIGKEYCOIN Core copyright is not removed by accident
    if (copyright_devs.find("BIGKEYCOIN Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BIGKEYCOIN Core developers";
    }
